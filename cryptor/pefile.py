...
class SectionStructure(Structure):
    ...
    def wipe_header_of_section(self, wipe_with="\x00"):
        self.pe.__data__ = self.pe.__data__[:self.__file_offset__] + wipe_with * self.sizeof() + self.pe.__data__[
                                                                                                 self.__file_offset__ + self.sizeof():]
...
class PE:
        ...
        if not s:
            if self.sections:
                lowest_rva = min([adjust_SectionAlignment(s.VirtualAddress,
                                                          self.OPTIONAL_HEADER.SectionAlignment, self.OPTIONAL_HEADER.FileAlignment) for s in
                                  self.sections])
    ...
    def is_driver(self):
        ...
        if hasattr(self, 'DIRECTORY_ENTRY_IMPORT'):
            ...
            #
            if set(('ntoskrnl.exe', 'hal.dll', 'ndis.sys', 'bootvid.dll', 'kdcom.dll')).intersection(
